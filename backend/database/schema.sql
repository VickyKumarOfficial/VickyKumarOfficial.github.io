...
-- Create AI feedback table (message-level and overall)
...
CREATE INDEX IF NOT EXISTS idx_ai_feedback_user_created_at ON public.ai_feedback(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_feedback_chat_created_at ON public.ai_feedback(chat_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_feedback_scope_created_at ON public.ai_feedback(feedback_scope, created_at DESC);
...
CREATE TRIGGER set_updated_at_ai_feedback
  ...
  FOR EACH ROW EXECUTE FUNCTION public.set_ai_feedback_updated_at();
